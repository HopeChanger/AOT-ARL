...
parent_dir = os.path.join(current_dir, '../../../gym-unrealcv-1.0')
...
class Rescale(gym.Wrapper):
    ...
    def rescale(self, x):
        ...
        new_obs = (((obs - self.mn_d) * (self.new_maxd - self.new_mind)) / self.obs_range) + self.new_mind
        return new_obs
...
class UnrealRescale(gym.ObservationWrapper):
    def __init__(self, env, args):
        ...
        self.num_obs = len(self.observation_space)
