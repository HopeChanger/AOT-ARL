...
from ENV.DigitalPose2D.target_move_with_obstacle import GoalNavAgent
...
class MultiAgentEnv(object):
    def __init__(self, reset_type=0,
        ...
        # obstacle
        self.obstacle_info = np.zeros((6, 4))
    ...
    def create_block(self):
        xp, yp = 3, 2
        self.block_range = [
            [(self.reset_area[1] - self.reset_area[0]) / xp * i + self.reset_area[0],
             (self.reset_area[1] - self.reset_area[0]) / xp * (i+1) + self.reset_area[0],
             (self.reset_area[3] - self.reset_area[2]) / yp * j + self.reset_area[2],
             (self.reset_area[3] - self.reset_area[2]) / yp * (j+1) + self.reset_area[2]]
            for i, j in [(0, 0), (0, 1), (1, 0), (1, 1), (2, 0), (2, 1)]
        ]

    def adjust_block(self, ratio):
        for i in range(len(self.block_range)):
            center = ((self.block_range[i][0] + self.block_range[i][1]) / 2,
                      (self.block_range[i][2] + self.block_range[i][3]) / 2)
            half_length = ((self.block_range[i][1] - self.block_range[i][0]) / 2,
                           (self.block_range[i][3] - self.block_range[i][2]) / 2)
            self.block_range[i][0] = center[0] - ratio * half_length[0]
            self.block_range[i][1] = center[0] + ratio * half_length[0]
            self.block_range[i][2] = center[1] - ratio * half_length[1]
            self.block_range[i][3] = center[1] + ratio * half_length[1]
    ...
    def random_reset(self):
        ...
        # reset obstacle
        self.create_block()
        self.adjust_block(ratio=0.8)
        self.obstacle_info = np.array([[
            float(np.random.randint(int(self.block_range[i][0]), int(self.block_range[i][1]))),
            float(np.random.randint(int(self.block_range[i][2]), int(self.block_range[i][3]))),
            float(np.random.randint(int(self.block_range[i][0]), int(self.block_range[i][1]))),
            float(np.random.randint(int(self.block_range[i][2]), int(self.block_range[i][3])))
        ] for i in range(len(self.block_range))])
        # reset targets
        ...
        for i in range(len(self.random_agents)):
            if 'Goal' in self.nav:
                self.random_agents[i].reset(self.target_pos_list[i].copy(), self.obstacle_info)
    ...
    def target_move(self, fix_step=None):
        ...
        if 'Random' in self.nav:
            ...
            for i in range(self.target_num):  # only one
                new_pose = self.random_agents[i].act()
                self.target_pos_list[i] = new_pose
    ...
    def render(self, file_name=None):
        ...
        for item in self.obstacle_info:
            line = plt.Line2D([item[0], item[2]], [item[1], item[3]])
            ax.add_artist(line)
        ax.add_artist(edge)
    ...
    def visible(self, cam_pos, target_pos):
        angle, dist = self.get_relative_position(cam_pos, target_pos)
        if dist > self.visual_distance or abs(angle) > self.visual_angle / 2:
            return False
        for item in self.obstacle_info:
            if self.cross_lines((cam_pos[:2], target_pos), ((item[0], item[1]), (item[2], item[3]))):
                return False
        return True
    ...
    def get_target_status(self):
        ...
        for i, cam in enumerate(self.cam_id):
            ...
            for j in range(self.target_num):
                if self.visible(cam_pos, self.target_pos_list[j]):
                    see_times[j] += 1
