...
from ENV.DigitalPose2D.multi_agent_env_with_obstacle import MultiAgentEnv
...
class PoseEnvBase(MultiAgentEnv):
    ...
    def get_add_memory_centralized_obs(self):
        ...
        for i, cam in enumerate(self.cam_id):
            ...
            for j in range(self.target_num):
                if self.visible(past_cam_pos, past_pos[j]) or j_now_visible[j] == 1:
    ...
    def get_obs(self):
        ...
        for i, cam in enumerate(self.cam_id):
            obs = []
            for j in range(self.target_num):
                if self.visible(cam_pos[i], self.target_pos_list[j]):
    ...
    def get_new_centralized_obs(self):
        ...
        for j in range(self.target_num):
            ...
            for i in range(len(cam_pos_list)):
                if self.visible(cam_pos_list[i], self.target_pos_list[j]):
                    find = 1
    ...
    def get_centralized_obs(self):
        ...
        normed_obstacle_info = (self.obstacle_info -
                                np.array([self.mean[0], self.mean[1], self.mean[0], self.mean[1]])) / self.scale
        ...
        for i in range(self.cam_num):
            ...
            for j in range(self.cam_num):
                if j != i:
                    now_obs = np.append(now_obs, cobs[j])
            now_obs = np.append(now_obs, normed_obstacle_info.reshape(-1))
            centralized_obs.append(now_obs)
        return centralized_obs
    ...
    def get_centralized_obs_size(self):
        ...
        obs_size = (self.target_num * 3 + 4 + self.cam_num) * self.cam_num + self.obstacle_info.size
